...
3. Debezium uses PostgreSQL logical replication (`pgoutput` plugin) to stream row changes for `public.countries`, `public.indicators`, `public.observations`, and `public.refugee_statistics` to Kafka topics named `wb.public.<table>`.
