...
2. **CDC** (`apps/cdc`, `apps/warehouse`): Debezium's Postgres connector reads the write-ahead log via logical replication (`pgoutput`) and emits change events to Kafka topics (`sr.public.<table>`, for all ten replicated tables). ClickHouse consumes these topics through `Kafka`-engine tables and a materialized view per table lands the parsed rows into `ReplacingMergeTree` tables (`raw_countries`, `raw_indicators`, `raw_observations`) — this is the near-real-time replication path required by the assessment, with no batch reload of Postgres involved.
...
**What's monitored:**
- **Pipeline health / freshness** — `sr_pipeline_cdc_lag_seconds`: seconds since the most recent CDC event landed in ClickHouse.
- **Replication completeness** — `sr_pipeline_cdc_lag_rows`: Postgres row count minus deduplicated ClickHouse row count, a direct backlog proxy.
- **Component reachability** — `sr_pipeline_scrape_success`: 0/1, whether both Postgres and ClickHouse answered this scrape.
...
- **The client-activity DAG cannot be backfilled, and that is a property of the source, not a missing feature.** The clients API serves current state only — a loan updated twelve times has one `updated_at`, with no as-of query and no change log — so a past interval has no history left to reconstruct. The watermark compounds it: keyed on resource name alone, it is global mutable state rather than a function of the run's interval, so a backfilled or cleared run reads "everything since now", finds nothing, and reports success. `catchup` is off and a run whose interval has already passed now raises `AirflowSkipException` rather than going green having done nothing. Making it genuinely backfillable needs two changes: an `updatedUntil` bound on the API so each run is a pure function of its `data_interval`, with the watermark kept only as a recovery floor for gaps; and, to answer "what did the portfolio look like on date X" at all, an append-only change log at the source exposed as `/changes?from=&to=`. Note the warehouse does not rescue this either — `ReplacingMergeTree` discards superseded versions on merge, so the history is not sitting in ClickHouse.
- **Client-activity ingestion trusts the source's `updated_at`.** If the source were to write a row with a timestamp earlier than the stored watermark — a clock skew, a backdated correction — that row would be skipped. The watermark itself only ever moves forward (`GREATEST` on write), so an out-of-order run cannot rewind it, but a backdated source row would still be missed. A production version would use a monotonic change sequence rather than a wall-clock timestamp, or re-read a short overlap window on each pass and rely on the upsert to absorb the duplicates.
