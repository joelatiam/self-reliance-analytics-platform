...
**If you already have the stack running from a revision before the project was
renamed** (compose project `worldbank-indicators-pipeline` → `self-reliance-analytics-platform`),
compose no longer recognises those containers as its own and will start a
second copy alongside them. Tear the old one down by name first:

```bash
docker compose -p worldbank-indicators-pipeline down -v
docker compose up -d
```

Ingestion applies `apps/ingestion/sql/` itself on every run, so Postgres schema
changes reach an existing volume without a reset. ClickHouse has no equivalent:
it only executes `apps/warehouse/init/` against an *empty* data directory, so
after changing anything there you need
...
```
http://localhost:4000/api/v1/loans?limit=500&updatedSince=2026-08-30T10:25:00.512Z&cursor=<meta.nextCursor>
```
