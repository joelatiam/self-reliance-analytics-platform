...
**If you have the stack running from before the rename**, it needs a full reset
rather than a restart. The compose project, the container names (`wb-*` →
`sr-*`), the databases (`worldbank` → `self_reliance`), the Postgres role, the
Kafka topic prefix and the Debezium replication slot all changed, so the old
volumes hold databases and topics under names nothing looks for any more:
...
Nothing is lost that matters — every row is re-fetched from the World Bank and
UNHCR APIs and regenerated by the clients API on the next run.

After that, schema changes look after themselves: ingestion applies
`apps/ingestion/sql/` and the CDC wait applies `apps/warehouse/init/` on every
run, both idempotently, so a table added later reaches an existing volume
without a reset.

Trigger a full pipeline run (or just wait for the 6-hourly schedule):

```bash
docker exec sr-airflow-scheduler airflow dags trigger country_indicators_pipeline
```

The client activity pipeline runs itself every ten minutes, but you can force a pass:

```bash
docker exec sr-airflow-scheduler airflow dags trigger client_activity_pipeline
```
...
docker exec sr-postgres psql -U sr_app -d self_reliance -c "SELECT count(*) FROM observations;"

# 2. Kafka — Debezium is publishing change events
docker exec sr-kafka /opt/kafka/bin/kafka-topics.sh --bootstrap-server localhost:9092 --list
...
docker exec sr-clickhouse clickhouse-client -q "SELECT count() FROM self_reliance.raw_refugee_statistics FINAL"
docker exec sr-clickhouse clickhouse-client -q "SELECT count() FROM self_reliance.raw_loans FINAL"

# 4. dbt staging/marts — transformations ran
docker exec sr-clickhouse clickhouse-client -q "SELECT count() FROM self_reliance.mart_country_indicators"
docker exec sr-clickhouse clickhouse-client -q "SELECT * FROM self_reliance.mart_country_program_context FORMAT Vertical"

# 5. Connector health directly from Kafka Connect's REST API
curl -s http://localhost:8083/connectors/sr-postgres-source/status
```

If Postgres and ClickHouse row counts (step 1 vs step 3) match, replication is caught up — this is exactly what the `sr_pipeline_cdc_lag_rows` metric in Grafana tracks continuously, with `sr_pipeline_cdc_lag_rows_by_table` breaking it down per table so one stalled topic cannot hide behind a healthy total.

For the incremental sources, `sr_pipeline_ingestion_watermark_age_seconds` is the faster signal: if a resource's watermark has not advanced in well over ten minutes, the pull has stalled.

```bash
# How far each incremental pull has got
docker exec sr-postgres psql -U sr_app -d self_reliance -c "SELECT resource, watermark, rows_ingested FROM ingestion_watermarks ORDER BY resource;"
```
...
|---|---|---|
| Postgres (OLTP) | `localhost:5433` (mapped from container port 5432 — 5432 is remapped to avoid clashing with a locally-installed Postgres) | `sr_app` / `sr_app_pw` (see `.env`) |
